# app/app.py
# Movie Recommender — dual-mode (Cloud + Local)

import os
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "")          # Cloud-safe
...
LOAD_EMB = os.getenv("LOAD_EMB", "0") == "1"     # only used in full/local if you insist
...
# Files for each mode (you will create the lite ones with the builder script below)
...
FULL_EMB     = os.getenv("FULL_EMB",     "movie_embeddings.npy")  # rarely used online

# Local fallbacks (if HF is unavailable)
DATA_CSV   = os.getenv("DATA_CSV",   "data/movies.csv")
FAISS_PATH = os.getenv("FAISS_PATH", "indexes/movie_index_ivfpq.faiss")  # or full index path
...
_FRANCHISE_KEYWORDS = {
    "marvel": ["avengers","iron man","captain america","thor","hulk","black widow","ant-man",
    ...
    "dc": ["batman","superman","wonder woman","aquaman","flash","justice league",
           "suicide squad","joker","shazam","black adam","dc","zatanna","cyborg",
           "green lantern","penguin","man of steel","superman"],
    "harry potter": ["harry potter","hogwarts","voldemort","dumbledore","hermione","ron weasley",
                     "fantastic beasts","grindelwald","quidditch","slytherin","gryffindor",
                     "hufflepuff","ravenclaw"],
    "lord of the rings": ["lord of the rings","frodo","gandalf","aragorn","middle earth","sauron",
                          "legolas","hobbit","bilbo","tolkien","elrond","mordor"],
    "star wars": ["star wars","skywalker","darth vader","yoda","jedi","sith","death star",
                  "grogu","mandalorian","obi-wan","kenobi","dooku","padmé","anakin","rey",
                  "bb-8","galactic empire"],
    "fast & furious": ["fast and furious","fast & furious","dom toretto","vin diesel","furious","f9",
                       "fast x","tokyo drift","ludacris","hobs and shaw","hobbs and shaw"],
    "transformers": ["transformers","bumblebee","optimus prime","megatron","autobot","decepticon",
                     "rise of the beasts"],
    "twilight": ["twilight","edward cullen","bella swan","jacob black","vampire","werewolf","breaking dawn"],
    "the hunger games": ["hunger games","katniss everdeen","peeta","panem","district","catching fire",
                         "mockingjay","snow"],
    "james bond": ["james bond","007","spectre","quantum of solace","skyfall","casino royale",
    ...
    "mission: impossible": ["mission impossible","mission: impossible","ethan hunt","imf","ghost protocol",
    ...
    "jurassic park": ["jurassic park","jurassic world","raptor","velociraptor","t-rex","indominus",
...
def detect_franchise(title: str, overview: str) -> str:
    ...
    matches = {}
    for f, kws in _FRANCHISE_KEYWORDS.items():
        for kw in kws:
            if kw in text:
                matches[f] = matches.get(f, 0) + 1
    return max(matches, key=matches.get) if matches else ""

def _ensure_franchise(df: pd.DataFrame) -> pd.DataFrame:
    if "franchise" not in df.columns: df["franchise"] = ""
...
def _find_query_movie_genres(df: pd.DataFrame, query_text: str) -> List[str]:
    if "title" not in df.columns or "genres" not in df.columns: return []
    q = query_text.strip().lower()
    if not q: return []
    exact = df[df["title"].str.lower() == q]
    if not exact.empty: return _split_genres(str(exact.iloc[0]["genres"]))
    ...
    if candidates:
        ...
        if not row.empty: return _split_genres(str(row.iloc[0]["genres"]))
    return []

def _find_query_movie_franchise(df: pd.DataFrame, query_text: str) -> str:
    if "title" not in df.columns or "franchise" not in df.columns: return ""
    q = query_text.strip().lower()
    if not q: return ""
    exact = df[df["title"].str.lower() == q]
    if not exact.empty: return str(exact.iloc[0]["franchise"]).strip()
    sample = df.nlargest(50000, "popularity") if "popularity" in df.columns and len(df) > 50000 else df
    titles = sample["title"].astype(str).tolist()
    cand = difflib.get_close_matches(query_text, titles, n=1, cutoff=0.92)
    if cand:
        row = sample[sample["title"] == cand[0]]
        if not row.empty: return str(row.iloc[0]["franchise"]).strip()
    return ""
...
def _hf_path(filename: str) -> str:
    return hf_hub_download(
        repo_id=HF_REPO_ID, filename=filename, repo_type=HF_REPO_TYPE,
        local_dir="data", local_dir_use_symlinks=False
    )

def _choose_files() -> tuple[str, str, Optional[str]]:
    """Return (parquet_file, index_file, emb_file) based on MODE + availability."""
    def exists_on_hf(fname: str) -> bool:
        try:
            _ = _hf_path(fname)
            return True
        except Exception:
    ...
    # MODE == auto: prefer lite, then full
...
def load_metadata() -> pd.DataFrame:
    try:
        df = _download_df_from_hf(PARQUET_FILE)
        ...
        if os.path.exists(DATA_CSV):
            ...
            st.error(
                ...
                f"Tried: {HF_REPO_ID}/{PARQUET_FILE} and {DATA_CSV}\n\nError: {e}"
            )
            st.stop()
...
def load_faiss():
    ...
    try:
        idx_path = FAISS_PATH if os.path.exists(FAISS_PATH) else _hf_path(INDEX_FILE)
...
def load_embeddings() -> Optional[np.ndarray]:
    if not EMB_FILE:  # not enabled in lite/auto-lite
        return None
    try:
        path = EMB_PATH if os.path.exists(EMB_PATH) else _hf_path(EMB_FILE)
...
def run_search(query_text: str, franchise_only: bool, safe_mode: bool, use_genres: bool, top_k: int) -> pd.DataFrame:
    ...
    if idx is None or len(idx) == 0:
        emb = load_embeddings()
        if emb is None:
            st.error("Search index unavailable (no FAISS / embeddings). Upload lite/full index or enable embeddings locally.")
            return pd.DataFrame()
...
with col_left:
    a,b = st.columns(2)
    with a: franchise_only = st.checkbox("Filter by franchise", value=False)
    with b: use_genres      = st.checkbox("Use genres of query movie", value=False)
with col_right:
...
if st.button("Recommend"):
    if not query.strip():
        ...
        with st.spinner("Searching…"):
            out = run_search(query.strip(), franchise_only, safe_mode, use_genres, top_k)
        if out.empty:
